      ...
      {open &&
        createPortal(
          <div
            ...
            className={`/* width, responsive */ /* safety clamp for tiny screens */ /* allow multiple lines */ fixed z-[9999] w-[86vw] max-w-[92vw] rounded-md border border-black/10 bg-white px-3 py-2 text-xs leading-tight break-words whitespace-normal text-[#222122] shadow-lg sm:w-[320px] sm:text-sm md:w-[360px] lg:w-[420px] dark:border-white/10 dark:bg-[#030303] dark:text-white`}
          >
            {message}
            {/* Arrow */}
            <span
              className={`absolute h-2 w-2 rotate-45 border border-black/10 bg-white dark:border-white/10 dark:bg-[#030303] ${coords.side === 'top' ? 'top-full left-1/2 -translate-x-1/2' : ''} ${coords.side === 'bottom' ? 'bottom-full left-1/2 -translate-x-1/2' : ''} ${coords.side === 'left' ? 'top-1/2 left-full -translate-y-1/2' : ''} ${coords.side === 'right' ? 'top-1/2 right-full -translate-y-1/2' : ''} `}
              aria-hidden="true"
            />
          </div>,
          document.body
